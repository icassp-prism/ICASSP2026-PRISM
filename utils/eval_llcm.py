"""LLCM evaluation helpers following same-camera filtering rules."""
...
def get_gallery_names(perm, cams, ids, trial_id, num_shots=1):
    """Select LLCM trial gallery names from the provided permutation matrix."""
...
def get_cmc(sorted_indices, query_ids, query_cam_ids, gallery_ids, gallery_cam_ids):
    ...
    for probe_index in range(sorted_indices.shape[0]):
        q_pid = query_ids[probe_index]
        q_cam_id = query_cam_ids[probe_index]

        g_pids_sorted = result[probe_index, :]
        g_cam_ids_sorted = cam_locations_result[probe_index, :]

        remove_mask = (g_pids_sorted == q_pid) & (g_cam_ids_sorted == q_cam_id)

        result_i = g_pids_sorted.copy()
        result_i[remove_mask] = -1
...
def eval_llcm(query_feats, query_ids, query_cam_ids, gallery_feats, gallery_ids, gallery_cam_ids, gallery_img_paths):
    """Evaluate one LLCM direction; caller swaps query/gallery for the reverse direction."""
