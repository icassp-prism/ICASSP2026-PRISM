"""High-level training loop wiring, checkpointing, and dataset-specific evaluation."""
...
from utils.eval_cm import eval_cmgroup
from configs.default.dataset import dataset_cfg
from configs.default.strategy import strategy_cfg
from utils.glpr import GLPR

def get_trainer(dataset, model, optimizer, lr_scheduler=None, logger=None, writer=None, non_blocking=False, save_dir="checkpoints", prefix="model", gallery_loader=None, query_loader=None, eval_interval=None, start_eval=None):
    """Attach Ignite callbacks for training logs, checkpoints, and final evaluation."""
    ...
    handler = ModelCheckpoint(save_dir, prefix, save_interval=eval_interval, n_saved=2, create_dir=True, save_as_state_dict=True, require_empty=False)
    ...
    def train_completed(engine):
        """Reload the best checkpoint and run the official evaluation protocol."""
        ...
        if dataset == "sysu":
            ...
            logging.info("no glpr:")
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="all", num_shots=1)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="all", num_shots=10)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="indoor", num_shots=1)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="indoor", num_shots=10)

            logging.info("glpr:")
            # GLPR is applied only after feature extraction, matching the paper evaluation flow.
            q_feats, g_feats = GLPR(q_feats, g_feats, k=20)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="all", num_shots=1)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="all", num_shots=10)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="indoor", num_shots=1)
            eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="indoor", num_shots=10)
            ...
            eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths)
            logging.info("visible to infrared")
            eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths)

            logging.info("glpr:")
            q_feats, g_feats = GLPR(q_feats, g_feats, k=7)
            logging.info("infrared to visible")
            eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths)
            logging.info("visible to infrared")
            eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths)
            ...
            logging.info("no glpr:")
            logging.info("infrared to visible")
            eval_llcm(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths)
            logging.info("visible to infrared")
            eval_llcm(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths)

            logging.info("glpr:")
            q_feats_glpr, g_feats_glpr = GLPR(q_feats, g_feats, k=15)
            logging.info("infrared to visible")
            eval_llcm(q_feats_glpr, q_ids, q_cams, g_feats_glpr, g_ids, g_cams, g_img_paths)
            logging.info("visible to infrared")
            eval_llcm(g_feats_glpr, g_ids, g_cams, q_feats_glpr, q_ids, q_cams, q_img_paths)

        elif dataset == "cmgroup_crop":
            perm = np.load(os.path.join(dataset_cfg.cmgroup_crop.data_root, "npy", "random_perm.npy"), allow_pickle=True)

            logging.info("no glpr:")
            eval_cmgroup(q_feats, q_ids, q_img_paths, g_feats, g_ids, g_cams, g_img_paths, perm, num_shots=1)
    ...
    def epoch_completed_callback(engine):
        ...
        msg = "Epoch[%d]\tSpeed: %.2f samples/sec" % (epoch, speed)
        for k in sorted(metric_dict.keys()):
            msg += "\t%s: %.4f" % (k, metric_dict[k])
            if writer is not None:
                writer.add_scalar("metric/{}".format(k), metric_dict[k], epoch)
        ...
        if evaluator and epoch % eval_interval == 0 and epoch > start_eval:
            ...
            if dataset == "sysu":
                perm = sio.loadmat(os.path.join(dataset_cfg.sysu.data_root, "exp", "rand_perm_cam.mat"))["rand_perm_cam"]
                mAP, r1, r5, _, _ = eval_sysu(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths, perm, mode="all", num_shots=1)
            elif dataset == "regdb":
                print("infrared to visible")
                mAP, r1, r5, _, _ = eval_regdb(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths)
                print("visible to infrared")
                mAP, r1_, r5, _, _ = eval_regdb(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths)
                r1 = (r1 + r1_) / 2
            elif dataset == "llcm":
                print("infrared to visible")
                mAP, r1, r5, _, _ = eval_llcm(q_feats, q_ids, q_cams, g_feats, g_ids, g_cams, g_img_paths)
                print("visible to infrared")
                mAP, r1_, r5, _, _ = eval_llcm(g_feats, g_ids, g_cams, q_feats, q_ids, q_cams, q_img_paths)
                r1 = (r1 + r1_) / 2
            elif dataset == "cmgroup_crop":
                perm = np.load(os.path.join(dataset_cfg.cmgroup_crop.data_root, "npy", "random_perm.npy"), allow_pickle=True)
                mAP, r1, r5, _, _ = eval_cmgroup(q_feats, q_ids, q_img_paths, g_feats, g_ids, g_cams, g_img_paths, perm, num_shots=1)
