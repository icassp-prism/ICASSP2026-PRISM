...
def train(cfg):
    ...
    logging.basicConfig(format="%(asctime)s %(message)s", filename=log_dir + "/" + cfg.log_name, filemode="w")
    ...
    train_loader = get_train_loader(dataset=cfg.dataset, root=cfg.data_root, sample_method=cfg.sample_method, batch_size=cfg.batch_size, p_size=cfg.p_size, k_size=cfg.k_size, random_flip=cfg.random_flip, random_crop=cfg.random_crop, random_erase=cfg.random_erase, color_jitter=cfg.color_jitter, padding=cfg.padding, image_size=cfg.image_size, num_workers=8)

    gallery_loader, query_loader = None, None
    if cfg.eval_interval > 0:
        gallery_loader, query_loader = get_test_loader(dataset=cfg.dataset, root=cfg.data_root, batch_size=64, image_size=cfg.image_size, num_workers=4)
    ...
    # Test loads retrieval weights directly; the model then returns BN-neck features.
...
if __name__ == "__main__":
    ...
    cfg = strategy_cfg
    # Merge defaults, YAML, dataset metadata, then CLI checkpoint/log options.
